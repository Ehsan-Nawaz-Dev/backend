export const replacePlaceholders = (template, data) => {
    ...
    const placeholders = {
        ...
        "{{price}}": order.total_price || order.total_price_set?.shop_money?.amount || "0.00",
        "{{subtotal}}": order.subtotal_price || order.subtotal_price_set?.shop_money?.amount || "0.00",
        "{{shipping_address}}": order.shipping_address?.address1 || order.customer?.default_address?.address1 || "",
